...
(function () {
  ...
  // trkn/disk — бинарные: [2b 0][2b number][2b total][2b 0]
  function trackBinary(num, total) {
    return new Uint8Array([0, 0, (num >> 8) & 255, num & 255, (total >> 8) & 255, total & 255, 0, 0]);
  }
  ...
  function buildIlstItems(opts) {
    ...
    if (opts.albumArtist) items.push(metaItem('aART', dataAtom(1, utf8(opts.albumArtist))));
    if (opts.year)   items.push(metaItem('©day', dataAtom(1, utf8(String(opts.year)))));
    if (opts.genre)  items.push(metaItem('©gen', dataAtom(1, utf8(opts.genre))));
    if (opts.trackNum) items.push(metaItem('trkn', dataAtom(0, trackBinary(opts.trackNum, opts.trackTotal || 0))));
    if (opts.discNum)  items.push(metaItem('disk', dataAtom(0, trackBinary(opts.discNum, 0))));
