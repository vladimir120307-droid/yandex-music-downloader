...
Расширение и десктоп позволяют выбрать качество (см. дропдаун «Качество» в попапе / «Я.Музыка качество» в десктопе):

| Опция | Размер | Звучит как | Кому |
|---|---|---|---|
| **Авто** (по умолчанию) | разный | Лучшее доступное | 🟢 Большинству — берёт FLAC если он есть у трека и у тебя Я.Плюс, иначе MP3 320 |
| **FLAC** (lossless) | 30-50 МБ | Идентично студии | 🎧 Меломанам с Я.Плюсом и хорошими наушниками (>$200)/колонками |
| **MP3 320 kbps** | ~10 МБ | Прозрачно для 95% ушей | Универсал — звучит как FLAC на обычной акустике |
| **MP3 192 kbps** | ~7 МБ | Слабые потери | Телефон, фоновое прослушивание |
| **AAC 256 kbps** | ~9 МБ | Лучше MP3 при том же битрейте | Apple-устройства |
| **AAC 128 kbps** | ~5 МБ | Заметные потери | Минимум, если совсем мало места |
| **Минимальный** | ~3-5 МБ | Худшее | Самое маленькое из доступного |

**FLAC доступен только для подписчиков Я.Плюса** и не для всех треков (только если у трека есть lossless-мастер). Без подписки или для треков без мастера — фоллбек на MP3 320.
